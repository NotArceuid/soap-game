import { powerOf10 } from "./PowerOf10";
import { repeatZeroes, trailZeroes } from "./RepeatZeroes";
import {
...
import {
  ...
  CountGeometricSeries,
  affordArithmeticSeries,
  CountArithmeticSeries,
...
export class Decimal {
  ...
  public static fromMantissaExponent(
    mantissa: number,
    exponent: number,
  ): Decimal {
   ...
   */
  public setMantissaExponent_noNormalize(
    mantissa: number,
    exponent: number,
  ): void {
    this.m = mantissa;
  ...
  public static fromMantissaExponent_noNormalize(
    mantissa: number,
    exponent: number,
  ): Decimal {
  ...
  public static fromNumber(value: number): Decimal {
    ...
    return cached === undefined
      ? Decimal.fromNumber_bypassCache(value)
      : cached;
  }
  ...
  public static fromString(value: string): Decimal {
    const cached = tryGetPooled(value);
    return cached === undefined
      ? Decimal.fromString_bypassCache(value)
      : cached;
  }
  ...
  public log(base: number): number {
    ...
    // We asCounte this to be true and thus only need to return a number, not a Decimal,
  ...
  public dp(): number {
    ...
    let e = 1;
    while (
      Math.abs(Math.round(mantissa * e) / e - mantissa) > ROUND_TOLERANCE
    ) {
      e *= 10;
      places++;
    }
   ...
   */
  public static CountGeometricSeries(
    numItems: number | Decimal,
    priceStart: DecimalSource,
    priceRatio: DecimalSource,
    currentOwned: number | Decimal,
  ): Decimal {
    return CountGeometricSeries(
      numItems,
      D(priceStart),
      D(priceRatio),
      currentOwned,
   ...
   */
  public static CountArithmeticSeries(
    numItems: DecimalSource,
    priceStart: DecimalSource,
    priceAdd: DecimalSource,
    currentOwned: DecimalSource,
  ): Decimal {
    return CountArithmeticSeries(
      D(numItems),
      D(priceStart),
      D(priceAdd),
      D(currentOwned),
  ...
  public static randomDecimalForTesting(absMaxExponent: number): Decimal {
    ...
    const exponent =
      Math.floor(Math.random() * absMaxExponent * 2) - absMaxExponent;
    return ME(mantissa, exponent);
  ...
  public mantissaWithDecimalPlaces(places: number): number {
    ...
    const rounded =
      Math.round(this.m * Math.pow(10, len - numDigits)) *
  ...
  public toExponential(places: number): string {
    ...
    return (
      rounded.toFixed(Math.max(len - numDigits, 0)) +
      "e" +
      (this.e >= 0 ? "+" : "") +
      this.e
    );
